...
- グループタブ・グループ詳細に通知バッジを追加
  - notification_providers: 未読チャット・未投票アンケート・未完了ToDo の件数プロバイダー
  - グループカード: グループ毎の通知件数をバッジ表示
  - グループ詳細の機能グリッド: チャット/ToDo/投票に個別バッジ表示
  - ボトムナビ「グループ」タブ: 全グループ合計の通知バッジ表示

- シフトペイントモード: カレンダー上でシフト種別を選択→日付タップだけで連続入力可能
  - カレンダー下部にシフト種別チップを常時表示
  - 種別選択中は日付タップで即座にシフトを割当（1日ずつフォームを開く必要なし）
  - 入力中バナー表示 + 「完了」ボタンでモード解除
